        ...
        <>
            ...
            <div className="flex flex-col md:flex-row w-full gap-10">
                <div className="md:w-1/3 flex md:flex-col gap-3 ">
                    {preguntas.map((p) =>
                        <div className="cursor-pointer flex flex-col md:flex-row items-center md:items-start bg-primary-ligtherblue rounded-3xl shadow-xl shadow-black w-full p-2 gap-2" key={p.id} onClick={() => { p.title == 'Proceso de compra' ? setSection('compra') : p.title == 'Envíos y entregas' ? setSection('envios') : setSection('cambios') }}>
                            <div>
                                {p.icon}
                            </div>
